...
class NBATeamRepository {
    ...
    private fun RecyclerViewData.toRoomObject(): LocalDataSourceSample {
    ...
    private fun List<LocalDataSourceSample>.toObjectDataSample(): List<RecyclerViewData> {
        return map { eachItem ->
            RecyclerViewData(
                teamName = eachItem.name,
                teamConference = eachItem.conference,
                teamRank = eachItem.rank,
                teamLogo = eachItem.logo
            )
        }
    }


    fun selectAllNBATeam(): LiveData<List<RecyclerViewData>> {
        return mNBATeamDao.selectAll().map { list ->
            list.toObjectDataSample()
        }
    }

    fun insertNBATeam(objectDataSample: RecyclerViewData) {
        mNBATeamDao.insert(objectDataSample.toRoomObject())
    }


    fun deleteAllNBATeam() {
        mNBATeamDao.deleteAll()
    }
